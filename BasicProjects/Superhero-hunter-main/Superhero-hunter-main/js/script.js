...
async function searchHeroes(textSearched) {
     // If there is no text written in the search bar, clear the results and return
     if (textSearched.length === 0) {
         searchResults.innerHTML = '';
         return;
     }
     ...
     // Construct the timestamp (ts) for the API request
     const ts = new Date().getTime();
 
     // Construct the hash for authentication
     const PRIVATE_KEY = 'ad79003cf7316d9bd72c6eda71d1c93d7e807e90';
     const hash = CryptoJS.MD5(ts + PRIVATE_KEY + PUBLIC_KEY).toString();
 
     // API call to get the data
     const response = await fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${textSearched}&apikey=${PUBLIC_KEY}&ts=${ts}&hash=${hash}`);
 
     // Check if the request was successful (HTTP status code 200)
     if (response.status === 200) {
         const data = await response.json(); // Convert the data into JSON format
         showSearchedResults(data.data.results); // Send the searched results to a function for display
     } else {
         // Handle the case where the API request is not successful (e.g., display an error message)
